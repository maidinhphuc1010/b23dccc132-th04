...
export function deleteGraduationDecision(id: string): ApiResponse<null> {
	const decisions = getLocalData(LOCAL_STORAGE_KEYS.GRADUATION_DECISIONS);
	const updatedDecisions = decisions.filter((decision: any) => decision.id !== id);
	setLocalData(LOCAL_STORAGE_KEYS.GRADUATION_DECISIONS, updatedDecisions);
	return { data: null, status: 200 };
}
